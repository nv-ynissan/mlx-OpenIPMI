...
if command -v mlxreg >/dev/null 2>&1; then
    ...
    if [ $? -eq 0 ] && [ -n "$mvcr_output" ]; then
        ...
        if [ -n "$power_value" ]; then
            ...
            echo "Error: Failed to extract power_sensor_value from MVCR register output."
        fi
    else
...
if [ -z "$soc_power" ]; then
    echo "SOC power not found using MVCR method, trying DebugFS method."
...
if [ -z "$power_envelope" ]; then
    echo "Power envelope not found using MVCR method, trying DebugFS method."
